...
*** NOTE!
***     I'm in the middle of working on a new modular version.
...
Version 2.19 09/20/21
- Updated for newest wow version TBC

Version 2.18 08/12/21
- Sorted names in Ratings
- Added ability to Sync Ratings with another person using the addon (Can take a while to complete depending on how much data there is to transfer)
----- NOTE that any whispers you send or receive may be delayed while Syncing.
- Added a message to the person your trying to Sync blacklist or Ratings, if they are using an old addon without the feature the sync will not happen and they will get a message telling them to update.
- In the "Players/Ratings" tab, any ratings NOT yours will say "Reported by:" and it will have the persons name on it when you put the mouse over the rating.
- Fixed the "Scrolling to top" when expanding or closing a tree item in Settings/Keys, Players/Blacklist and Players/Ratings. Whenever clicking an Item the list stays in place and no longer goes to the top of the list.
- Fixed some items are not displaying names in Ratings/Blacklist/keys.
- Fixed the "Need" not showing up while searching for a group to join.
- Fixed the bug where you couldnt open a Blacklist item after viewing keys in settings.
- Removed the ClearABlackListDisplay function
- Added Submenu to 'Targeted' character as well as "Friends List" popup (Tabs 'Who', 'Guild' and 'Raid')
 ----- This will definitly help if a guild raid leader is creating a premade raid for future runs, or you are with a group and agree to finish a run in a future date.

Version 2.17 07/11/21
- Removed the Extra - Copied line that caused EOF Error

Version 2.16 07/11/21
- Fixed bug with Ratings when you are in the instance and invite someone new.
- Moved anyone in your group to show at the top of the Players/Ratings tab for easier finding marked with a "*".
- Added a notification popup whenever ANYONE you had rated in the past joins your group. A popup will show notifying you of this person and their average rating.
----- Ratings have to be enabled for this to show.
----- It is up to you to go into the Players/Ratings tabs to see your notes and instances on this person.

Version 2.15 07/06/21
- Corrected map for Draenei
- SHIFT/CLICK on any search results will open a whisper window to that individual for faster communication.
- Option added in General tab to disable/enable Rating system
- ANY character with a rating will show stars on the right of their search. This will inform you that you ran with them before and what experience you had.
- Rating system saves on players you only rate, others are not saved
- Timer added to capture all players who are added to group or if you join a group
- Rating will only show when you leave a group
- Fixed Saved Searches frame offset by 4
- Ratings can be viewed under PLAYERS/RATINGS
----- When a player's name is clicked in the list, it will open up all runs you saved.
----- the runs will list any notes you made on that run and all instances you were in while they were in the party
----- FIRST row: RACE, CLASS name number of runs and average rating of all Runs
----- SUBROWS:   Date/time of run, level of character at time of run and rating given for specific runs.
----- MOUSEOVER: Shows Comments made for specific run and all instances you wan while in that group.

Version 2.14 06/28/21
- Disabled the Attunement display for the raids and instances trying to diagnose why some raids are not showing.
- Corrected an LUA error

Version 2.13 06/23/21
- Added filter by roles, if no role is selected then all available roles are shown. If no role was detected in chat, then any role will be assumed.

Version 2.12 06/21/21
- Added Level range for Instances under "Settings/Keys" This will allow you to personalize the range you feel the instance should be.
- All keys or information that have changed will show at the top of the key list with a * next to the name of the instance
- Added "Hero" to Heroic Keywords
- Added a few more keywords to instances.
- Corrected the Frames of instances to properly show in the window
- Changed / Shortened up the instance names to be used when broadcasting instead of the full name

Version 2.11 06/20/21
- Detects the "Questie" addon TBC quests in channels and Shows them under the Quest pulldown.
- I will ADD the ability to use the search Quests once I figure out the best aproach to using pulldowns and adding 1430 quests.
- Right now, there are no level restrictions with Quests filtering

Version 2.10 06/16/21
----- PLEASE NOTE ***
- This was sent up earlier then expected due to some clients having issues with instances. There may be a few bugs.
-
- Made the notification text lighter so it is easier to read (yellow instead of red)
- Added code to remove spacing in channel list
- added submenu item to rate a specific person when you right click on the name.
- Updated instance levels
- Updated keywords for instances to catch 99% of chat. Most of instances missed are misspelled.

Version 2.09 06/16/21
- Fixed pulldown, you can now select multiple instances again
- Removed the "Debug chat" from the chat box I forgot to remove.
- Resolved issue with Raids/PVP not being able to be selected.
- Started working on a Rating system to rate those you grouped with.

Version 2.08 06/15/21
- Blacklist being synced are now invisible to the users. All you will see are status in the chat.

Version 2.07 06/11/21
- Changed the way Heroics are detected... Increased results
- Added hc for heroic search
- Changed Escape to Old Hillsbrad
- Added 2 local strings for Blacklist sharing
- Added ability to SYNC your blacklist with someone else. Right click on the persons name and click LFG113/Sync Blacklist
----- The way this works, It will whisper the person, if you get a proper response then it will sync. If you do not get the correct response, then no spam/sync happens
- TESTING Keywords for instances (In Settings/Keys)
----- PLEASE NOTE ***
----- ANY Instance with a * in the list means it has been changed in one way or another. Click on the name and then DEFAULT to return it to the original/unchanged version.
----- INSTANCE KEY: This is the key the addon uses to assign your changes. You can not change this.
----- PULLDOWN: This is what you would see when you use the pulldown to select instances.
----- BROADCAST: This is what you want others to see when you broadcast the search.
----- Click on the instance you want to change, it will expand to show the current keywords.
----- Click on the "+" button to add a keyword
----- Click on the "x" button next to a keyword to remove it.
----- BUG: So far, the only known bug is if you have an instance selected and change its pulldown or broadcast, the instance will continue to be displayed as the previous.
-----      To correct this, just Reset your search.


Version 2.06 06/07/21
- Added option to only allow notifications while solo and searching (Settings/Display) as opposed to all the time when solo.
- Organized the Settings/Display
- Added a new Placeholder for string entries: {cs} - Your current talent spec

Version 2.05 06/06/21
- Added context, labels and pupup messages for new menu items for available language packs..
- Added color to RAIDWARNING (White is Character, Red is the text)

Version 2.04 06/06/21
- Added and trying a new Notification system using the RAID WARNING when a new Group shows in the interface.
----- PLEASE NOTE ***
----- This can and will be very anoying if you are viewing ALL groups (it will constantly go off on a busy server).
----- This system is designed to let you know when there is a new posting for a group that fits your current criteria.
----- Notification AND sound is OFF by default, you can turn it on in Settings/Display Tab.
----- Notification will only work when not in group and when the a leader of a group searching.
- For those that do not know, the CUSTOM pulldown will show you EVERY RAID/Instance and PVP available to you.
- Corrected an issue with the interface using 2 words from the communication pack (and / or).
- Fixed an error with Gruuls Lair giving a lua error (Thanks to vbrokop pointing this out)

Version 2.03 05/31/21
- Moved LBRS to Dungeons with a cap at 10 Players
- Changed the way the interface works.
----- It will show ALL available instances/raids/pvp (whatever you have selected) via your level when nothing is selected for filtering.
----- It will start to filter once you select any instance/raid/pvp.

Version 2.02 05/30/21
- Changed classes to work with different clients (like French, thanks to laysson)

Version 2.01 05/23/21
- Added Heroic Instances
- Added reputation requirements to TBC Heroics that show in pulldown
- Added attunements to TBC RAIDS that show in pulldown

Version 2.0 05/20/21
- Client will work on both CLASSIC ERA and TBC clients.
- Activity AND/OR were not disappearing when you expand the viewing area.
- Updated for TBC, changed all frames to inherit backdrops (A change implemented in the 9.0+ client)
- Added Dungeons and raids for TBC
- Organized the pull downs according to client using. TBC will not show up in interface while using Vanilla, but Vanilla WILL show up in TBC.
NOTE *There is one issue however, the Vanilla pull down may be to big for the screen, you will have to drag the LFG down a little bit until the pull down shows all the content.

Version 1.17 04/22/21
- Changed version  to newest wow client released
