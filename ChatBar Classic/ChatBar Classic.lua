...
Edited: Foxylittlething, aNNiPAk, Yarko
...
- updated "IDDQD" skin
- new skin "Moon", by rikka0612
- fixed "lock" feature, now it must work correctly and save frame location
- fix some typos
- some localizations additions
v 3.15.1b (Foxylittlething)
- new feature "lock position"
- BfA compatibality update
- update enGB localization
- update ruRU localization
v 3.14.2b (Foxylittlething)
- new skin IDDQD
- update enGB localization
- update ruRU localization
v 3.14.1b (Foxylittlething)
...
ChatBar_AltArtDirs = { "SkinSolid", "SkinGlass", "SkinSquares", "SkinIDDQD",  "SkinMoon"};
...
function ChatBar_OnDragStart(self)
	if (not self.isSliding and not ChatBar_isLock) then
		--local x, y = GetCursorPosition();
		--self:ClearAllPoints();
...
function ChatBar_CreateFrameMenu()
	...
	--Lock Main Frame
	local info = {};
	info.text = CHATBAR_MENU_MAIN_LOCK;
	info.func = ChatBar_Lock;
	if (ChatBar_isLock) then
		info.checked = 1;
	end
	UIDropDownMenu_AddButton(info, UIDROPDOWNMENU_MENU_LEVEL);
end
...
function ChatBar_Lock()
	ChatBar_isLock = not ChatBar_isLock;
end

function ChatBar_UpdateButtonArt(buttonIndex, artDir)
	_G["ChatBarFrameButton"..buttonIndex.."UpTex_Spec"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Up_Spec");
	_G["ChatBarFrameButton"..buttonIndex.."DownTex_Spec"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Down_Spec");
	_G["ChatBarFrameButton"..buttonIndex.."Flash"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Glow_Alpha");
	
	_G["ChatBarFrameButton"..buttonIndex.."Center"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Center");
	_G["ChatBarFrameButton"..buttonIndex.."Background"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_BG");
	
	_G["ChatBarFrameButton"..buttonIndex.."UpTex_Shad"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Up_Shad");
	_G["ChatBarFrameButton"..buttonIndex.."DownTex_Shad"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Down_Shad");
	_G["ChatBarFrameButton"..buttonIndex.."Highlight"]:SetTexture("Interface\\AddOns\\ChatBar Classic\\Skins\\"..artDir.."\\ChanButton_Glow_Alpha");
end
