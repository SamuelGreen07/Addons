...
local timerSharedFateCD				= mod:NewNextCountTimer(29, 179909, nil, "-Tank", 2, 3, nil, DBM_COMMON_L.DEADLY_ICON)--29-31
...
local timerDigest					= mod:NewCastTimer(40, 181295, nil, nil, nil, nil, nil, DBM_COMMON_L.DEADLY_ICON, nil, 1, 8)
...
function mod:OnCombatStart(delay)
	...
	if self:IsMythic() then
		...
		timerShadowofDeathCDHealer:Start(21-delay, "2x"..DBM_COMMON_L.HEALER_ICON)
	else
		...
		timerShadowofDeathCDDps:Start(2-delay, numDpsPlayers.."x"..DBM_COMMON_L.DAMAGE_ICON, 1)
		timerShadowofDeathCDTank:Start(13-delay, "1x"..DBM_COMMON_L.TANK_ICON, 2)
		timerShadowofDeathCDHealer:Start(30-delay, numHealerPlayers.."x"..DBM_COMMON_L.HEALER_ICON, 3)
	end
...
function mod:OnSync(msg)
	...
	elseif msg == "ShadowofDeathApplied" then
		...
		if self:IsMythic() then
			if count == 1 or count == 4 or count == 5 then--DPS 4x (3 timers)
				timerShadowofDeathCDDps:Start(27, "2x"..DBM_COMMON_L.DAMAGE_ICON)
			elseif count == 2 then--Tank 2x (1 timer)
				timerShadowofDeathCDTank:Start(60, "1x"..DBM_COMMON_L.TANK_ICON)
			elseif count == 3 then--Healer 2x (1 timer)
				timerShadowofDeathCDHealer:Start(45, "2x"..DBM_COMMON_L.HEALER_ICON)
			end
		else
			if count == 1 or count == 4 then--DPS 3x (2 timers)
				...
				timerShadowofDeathCDDps:Start(36, numPlayers.."x"..DBM_COMMON_L.DAMAGE_ICON)
			...
			elseif count == 3 and playersCount > 10 then--Healer 2x (1 timer). Only gets a 2nd one if > 10 players
				...
				timerShadowofDeathCDHealer:Start(36, numPlayers.."x"..DBM_COMMON_L.HEALER_ICON)
			end
		end
	elseif msg == "FeastEnded" then
		...
		if self:IsMythic() then
			timerShadowofDeathCDDps:Start(2, "2x"..DBM_COMMON_L.DAMAGE_ICON)
			timerShadowofDeathCDTank:Start(9, "1x"..DBM_COMMON_L.TANK_ICON)
			timerShadowofDeathCDHealer:Start(21, "2x"..DBM_COMMON_L.HEALER_ICON)
			if self.Options.InfoFrame then
			...
			if playersCount >= 20 then numHealerPlayers = 2 end--2 healers 20 players or over
			timerShadowofDeathCDDps:Start(2, numDpsPlayers.."x"..DBM_COMMON_L.DAMAGE_ICON)
			timerShadowofDeathCDTank:Start(13, "1x"..DBM_COMMON_L.TANK_ICON)
			timerShadowofDeathCDHealer:Start(30, numHealerPlayers.."x"..DBM_COMMON_L.HEALER_ICON)
		end
