...
function Details:SendPlayerClassicInformation (targetPlayer)
	...
	if (spec and spec [2] >= MIN_SPECS) then
		...
		if (targetPlayer) then
			_detalhes:SendCommMessage (CONST_DETAILS_PREFIX, _detalhes:Serialize (CONST_ANSWER_TALENTS, UnitName("player"), GetRealmName(), _detalhes.realversion, UnitGUID ("player"), 0, talentsSelected, Details.playerClassicSpec.specs), "WHISPER", targetPlayer)
	...
	if (playerTalents) then
		...
		if (targetName) then
			...
			if (not DetailsTalentFrame.showAnimation) then
				...
				DetailsFramework:CreateAnimation (animHub, "ALPHA", 1, .1, 0, .2)
				DetailsFramework:CreateAnimation (animHub, "ALPHA", 2, 2, .2, .5)
				DetailsFramework:CreateAnimation (animHub, "ALPHA", 3, .3, .5, 1)
